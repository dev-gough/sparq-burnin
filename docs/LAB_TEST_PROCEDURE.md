...
| LT-05 | Very long duration test | 1 | PASS | 87 h × 35 inverters (2026-07-31 17:30 → 08-04 08:30 EDT). All 35 acked within 17 s of test end (~7 s total upload). ~5,340 samples each; both month boundaries (UTC flip 2026-08-01T00:00Z and wall-midnight 04:00Z) perfectly monotonic at 1-min cadence; zero duplicates. NOTE: software reported `overall_status = RETEST` — stores/uploads fine but the dashboard has no first-class RETEST support (badge falls back to "INVALID" text, no filter chip, counted in summary denominators) — follow-up work item |
