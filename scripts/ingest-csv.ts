...
class CSVIngester {
  ...
  async findTestIdForDataFile(dataFileName: string): Promise<number | null> {
    // Extract serial number from filename (format: inverter_SERIAL_DATE.csv)
    const match = dataFileName.match(/inverter_(\d+)_/);
    if (!match) {
      console.warn(`Could not extract serial number from filename: ${dataFileName}`);
      return null;
    }
    
    const serialNumber = match[1];
    
    // Find the most recent test for this inverter
    const query = `
      SELECT t.test_id 
      FROM Tests t 
