/**
 * Pins the proxy's public-route list (plan item 2.5).
 *
...
// providers). The proxy module only needs it to wrap its callback.
vi.mock('@/lib/auth', () => ({
  auth: (handler: unknown) => handler,
}))

import { PUBLIC_PREFIXES, PUBLIC_EXACT, isPublicPath, config } from '@/proxy'
...
describe('public route list', () => {
  ...
  it('matcher literals agree with isPublicPath for public API paths', () => {
    // Public API paths must be excluded by the matcher (proxy never runs).
    for (const p of publicPaths.filter((p) => p.startsWith('/api/'))) {
      expect(proxyRunsOn(p), p).toBe(false)
    }
  })

  it('matcher literals agree with isPublicPath for protected paths', () => {
    for (const p of protectedPaths) {
      expect(proxyRunsOn(p), p).toBe(true)
    }
  })
